...
def download_image(driver, url, path, loop):
    # HTTPヘッダ作成
...
def scraping(QUERY, LIMIT_DL_NUM, SAVE_DIR, FILE_NAME, TIMEOUT, ACCESS_WAIT, RETRY_NUM, DRIVER_PATH):
    ...
    tmb_elems = driver.find_elements(By.CSS_SELECTOR, '#islmp img')
    ...
    imgframe_elem = driver.find_element(By.ID, 'islsp')
...
def main():
    QUERY = os.getenv('QUERY')                        # 検索ワード
    LIMIT_DL_NUM = int(os.getenv('LIMIT_DL_NUM'))                          # ダウンロード数の上限
    SAVE_DIR = os.getenv('SAVE_DIR')                        # 出力フォルダへのパス（フォルダがない場合は自動生成する）
    FILE_NAME = os.getenv('FILE_NAME')                          # ファイル名（ファイル名の後ろに０からの連番と拡張子が付く）
    TIMEOUT = int(os.getenv('TIMEOUT'))                                # 要素検索のタイムアウト（秒）
    ACCESS_WAIT = int(os.getenv('ACCESS_WAIT'))                             # アクセスする間隔（秒）
    RETRY_NUM = int(os.getenv('RETRY_NUM'))                               # リトライ回数（クリック、requests）
    DRIVER_PATH = os.getenv('DRIVER_PATH')       # chromedriver.exeへのパス
    scraping(QUERY, LIMIT_DL_NUM, SAVE_DIR, FILE_NAME, TIMEOUT, ACCESS_WAIT, RETRY_NUM, DRIVER_PATH)
